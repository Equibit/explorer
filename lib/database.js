...
const rpc = {
  ...
  getblockcount: {
    ...
    missing (data) {
      console.log(`GET block count: ${data}`)
      return data ? false : rpcOnError('getblockcount', 'not all data found')
    }
  },
  ...
  getblock: {
    ...
    async set (raw) {
      return Block.create([{
        ...raw,
        fulltx: await Promise.all(
          raw.tx.map(txid => lib.getRawRpc('getrawtransaction', [ txid, 1 ]))
        ).then(txs => {
          console.log(txs)
          return txs
        })
