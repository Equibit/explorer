const mongoose = require('mongoose'),
  ...
  { promisify, prettyPrint, renameProp, requestp, wait } = require('./util')
...
const rpc = {
  ...
  getnetworkhashps: {
    ...
    missing (data) {
      ...
      return ready || rpcOnError('getnetworkhashps', 'not all data found')
    }
  },
  ...
  getmininginfo: {
    ...
    missing (data) {
      ...
      console.log(ready)
      return ready || rpcOnError('getmininginfo', 'not all data found')
    }
  },
  ...
  getdifficulty: {
    ...
    missing (data) {
      return data || rpcOnError('getdifficulty', 'not all data found')
    }
  },
  ...
  getconnectioncount: {
    ...
    missing (data) {
      return data || rpcOnError('getconnectioncount', 'not all data found')
    }
  },
  ...
  getblockcount: {
    ...
    missing (data) {
      return data || rpcOnError('getblockcount', 'not all data found')
    }
  },
  ...
  getblockhash: {
    ...
    missing (data) {
      return data.length === 1 || rpcOnError('getblockhash', 'not all data found')
    }
  },
  ...
  getblock: {
    ...
    missing (data) {
      ...
      return ready || rpcOnError('getblock', 'not all data found')
    }
  },
  ...
  getrawtransaction: {
    ...
    missing (data) {
      ...
      return ready || rpcOnError('getrawtransaction', 'not all data found')
    }
  },
  ...
  getpeerinfo: {
    ...
    missing (data) {
      return data || rpcOnError('getpeerinfo', 'not all data found')
    }
  },
  ...
  gettxoutsetinfo: {
    ...
    missing (data) {
      ...
      return ready || rpcOnError('gettxoutsetinfo', 'not all data found')
    }
  },
...
module.exports = {
  ...
  async updateBlock(updater, identifier) {
    ...
    if (!identifier) {
      if (updater.hasOwnProperty('height')) identifier = 'height'
      else if (updater.hasOwnProperty('hash')) identifier = 'hash'
      else throw Error('Cannot identify target block')
    }

    identifier = (identifier instanceof Object)
      ? identifier : { [identifier]: data[identifier] }
    updater = (updater instanceof Function) ? updater : () => updater

    let [ err, block ] = await promisify(Block.findOne.bind(Block), identifier)
    if (err || !block) {
      await Block.create([ await updater() ])
    } else {
      await promisify(Block.updateOne.bind(Block), identifier, await updater(block))
    }
  },
